...
（1）查找DOM元素： document.getElementById('root') 查找到HTML中的<div id="root"></div>元素。
（2）创建根： ReactDOM.createRoot(element)根据找到的元素创建一个React根。
（3）渲染组件： 根的render方法被调用并传递了<App />，启动组件树的渲染并将它们插入文档中的<div id="root">，<App />就是主应用组件，也就是App.js,根据代码引入的那块也能找打对应的App.js位置。
